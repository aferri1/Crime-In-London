...
# # merge the dfs to get lsoa boundary data with crime data
# merged_data = lsoa_geo.merge(LSOA_2021, left_on='LSOA11CD', right_on='LSOA Code')

# merged_data['Log_Total_2021'] = np.log1p(merged_data['Total_2021'])  # log scale the crime data

# # Create a base map
# m = folium.Map(location=[51.5074, -0.1278], zoom_start=12)  # longitude and latitude over central london
...
#     name='choropleth',
#     data=merged_data,
#     columns=['LSOA Code', 'Log_Total_2021'],
#     key_on='feature.properties.LSOA Code',
#     fill_color='OrRd',
#     fill_opacity=0.7,
#     line_opacity=0.2,
...
# m.save('london_crime_heatmap_test.html')



# # population density heatmap:

# # merge the dfs to get lsoa boundary data with population data
# merged_data = lsoa_geo.merge(population_data, left_on='LSOA11CD', right_on='LSOA Code')

# # remove columns and convert to numerical:
# merged_data['People per Sq Km'] = merged_data['People per Sq Km'].str.replace(',', '')
# merged_data['People per Sq Km'] = pd.to_numeric(merged_data['People per Sq Km'], errors='coerce')

# merged_data['Log_pop_dens'] = np.log1p(merged_data['People per Sq Km'])  # log scale the crime data

# # list of london LSOAs:
# london_lsoa_codes = crime_data['LSOA Code'].unique().tolist()


# # make it only london lsoas:
# merged_data = merged_data[merged_data['LSOA11CD'].isin(london_lsoa_codes)]
...
#     name='choropleth',
#     data=merged_data,
#     columns=['LSOA Code', 'Log_pop_dens'],
#     key_on='feature.properties.LSOA Code',
#     fill_color='OrRd',
#     fill_opacity=0.7,
#     line_opacity=0.2,
#     legend_name='Log Population Density 2020'
# ).add_to(m)

# folium.LayerControl().add_to(m)

# m.save('london_pop_dens_heatmap_2020_log.html')
